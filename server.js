...
//  - Simple in-memory upload queue with progress/ETA + speed
...
//  - Mobile-friendly UI + Capacity graph (donut + bar)
//
// Env:
// PORT=8080
...
// WEBDAV_USER=skypool
// WEBDAV_PASS=skypool
// (optional) PGSSLMODE=disable
//
// package.json deps: express, multer, pg, googleapis, uuid, dotenv, basic-auth, mime-types
...
async function driveUpload({ client, name, mimeType, data }) {
  ...
  const res = await drive.files.create(
    { requestBody: { name }, media: { mimeType, body: data } },
    { headers: { 'X-Upload-Content-Type': mimeType } }
  );
  return res.data; // { id, name }
}
async function driveDelete({ client, fileId }) {
  const drive = google.drive({ version: 'v3', auth: client });
  await drive.files.delete({ fileId }).catch(()=>{});
}

// ---------- Utils ----------
const BAD_NAME_CHARS = new RegExp('[\"\\\\\\r\\n]', 'g'); // safer than inline literal in some pipelines
function safeFileName(name) {
  return (path.basename(name || 'file')).replace(BAD_NAME_CHARS, '_') || 'file';
}
...
const jobs = new Map(); // id -> {id, filename, size, status, progress, error, createdAt, tmpPath, doneBytes, totalBytes, startedAt, finishedAt, speedBps, etaSeconds, _markAt, _markBytes}
const queue = [];
let working = false;
...
async function processJob(job){
  ...
  await query(
    'INSERT INTO manifests(id,original_name,original_mime,size,sha256,chunk_size_mb,created_at) VALUES($1,$2,$3,$4,$5,$6,$7)',
    [manifestId, job.filename, contentTypeFor(job.filename), size, fileSha, chunkMB, Date.now()]
  );
  ...
  for (const p of parts){
    ...
    const meta = await driveUpload({
      client,
      name: p.name,
      mimeType: 'application/octet-stream',
      data: fssync.createReadStream(p.path)
    });
    await query(
      'INSERT INTO parts(manifest_id,idx,size,sha256,account_id,drive_file_id,drive_file_name) VALUES($1,$2,$3,$4,$5,$6,$7)',
      [manifestId, p.idx, p.size, p.sha256, acc.id, meta.id, meta.name || p.name]
    );
    aIdx++;
    job.doneBytes += p.size;
    job.progress = Math.round((job.doneBytes / job.totalBytes) * 100);
    ...
    if (dt > 0.5) {
      ...
      job.speedBps = job.speedBps ? (0.7 * job.speedBps + 0.3 * inst) : inst;
...
// ---------- UI (animated + mobile + capacity graphs) ----------
app.get('/', adminAuth, async (req,res)=>{
  const accounts = await getAccounts();

  // Try Google quota; fall back to 15 GB/account
  ...
  for (const a of accounts) {
    try {
      ...
      usageList.push({ name: a.display_name||a.email, limit:0, usage:0 });
    }
  }

  // Ground truth used (our pool)
  const totalRow = await query('SELECT COALESCE(SUM(size),0) AS total FROM manifests');
  const poolUsedBytes = Number(totalRow[0]?.total || 0);
  const fallbackPerAcc = 15 * 1e9;
  const poolCapacityBytes = totalLimit > 0 ? totalLimit : accounts.length * fallbackPerAcc;
  const usedPct = poolCapacityBytes>0 ? Math.min(100, Math.round((poolUsedBytes/poolCapacityBytes)*100)) : 0;

  // donut path
  const r = 36; const c = 2 * Math.PI * r; const dash = (usedPct/100) * c; const offset = c - dash;
  ...
  <style>
    ...
    .cap{display:grid;grid-template-columns:100px 1fr;gap:16px;align-items:center}
    .donut{width:100px;height:100px}
    .capbar{height:14px;background:#0f233f;border-radius:999px;overflow:hidden;border:1px solid #1a2e55}
    .capbar>i{display:block;height:100%;background:linear-gradient(90deg,#3b82f6,#60a5fa);width:${usedPct}%}
    .accList{display:flex;flex-wrap:wrap;gap:8px;margin-top:8px}
    .accBadge{background:#0e1d35;border:1px solid #1e335a;border-radius:8px;padding:6px 8px;color:var(--mut);font-size:12px}
  </style></head>
  <body>
    <header>
      <div class="ocean">
    ...
    <main class="wrap grid">
      ...
      <section class="card">
        <h3>Capacity</h3>
        <div class="cap">
          <svg class="donut" viewBox="0 0 100 100" width="100" height="100" role="img" aria-label="Pool usage">
            <circle cx="50" cy="50" r="36" stroke="#0f233f" stroke-width="12" fill="none" />
            <circle cx="50" cy="50" r="36" stroke="#4da3ff" stroke-width="12" fill="none"
              stroke-linecap="round"
              style="transform:rotate(-90deg); transform-origin:50% 50%; stroke-dasharray:${(2*Math.PI*36).toFixed(2)} ${(2*Math.PI*36).toFixed(2)}; stroke-dashoffset:${offset.toFixed(2)}" />
            <text x="50" y="54" text-anchor="middle" font-size="16" fill="#e8f0ff">${usedPct}%</text>
          </svg>
          <div>
            <div class="capbar"><i></i></div>
            <div class="mut" style="margin-top:6px">${(poolUsedBytes/1e9).toFixed(1)} / ${(poolCapacityBytes/1e9).toFixed(1)} GB pooled</div>
            <div class="accList">
              ${usageList.map(u=>`<span class="accBadge">${safe(u.name)} — used ${(u.usage/1e9).toFixed(1)} GB${u.limit?(' / '+(u.limit/1e9).toFixed(1)+' GB'):' / 15 GB est.'}</span>`).join('')}
            </div>
          </div>
        </div>
...
app.get('/api/jobs', adminAuth, (req,res)=>{
  const arr = [...jobs.values()]
    ...
    .slice(0,100)
    .map(j=>({
      id:j.id, filename:j.filename, size:j.size, status:j.status,
      progress:j.progress||0, error:j.error||null,
      doneBytes:j.doneBytes||0, totalBytes:j.totalBytes||j.size||0,
      speedBps:j.speedBps||0, etaSeconds:j.etaSeconds??null
    }));
  res.json(arr);
});
...
app.get('/oauth2/callback', async (req,res)=>{
  try{
    ...
    const id = uuidv4();
    await query('INSERT INTO accounts(id,email,display_name,refresh_token,created_at) VALUES($1,$2,$3,$4,$5)',
      [id,email,name,tokens.refresh_token,Date.now()]);
    res.redirect('/');
...
app.post('/upload', adminAuth, upload.single('file'), async (req,res)=>{
  try{
    const st = await fs.stat(req.file.path);
    const id = uuidv4();
    jobs.set(id, {
      id,
      filename: safeFileName(req.file.originalname||'file'),
      size: st.size,
      status:'queued',
      progress:0,
      createdAt: Date.now(),
...
app.post('/delete', adminAuth, express.urlencoded({extended:true}), async (req,res)=>{
  ...
  try{
    ...
    for(const p of parts){
      try{ const { client } = await clientForAccount(p.account_id); await driveDelete({ client, fileId: p.drive_file_id }); }catch{}
    }
    ...
    return res.redirect('/?err=delete');
  }
});

// Delete (GET fallback → auto-submit POST)
...
app.get('/download', async (req,res)=>{
  try{
    ...
    const name=safeFileName(m.original_name||'file');
    const ctype=m.original_mime||contentTypeFor(name);
...
function davAuth(req,res,next){
  const c=basicAuth(req);
  if(!c||c.name!==DAV_USER||c.pass!==DAV_PASS){
    res.set('WWW-Authenticate','Basic realm="SkyPool DAV"');
...
app.all('/dav*', davAuth);
app.all('/dav', davAuth, async (req,res,next)=>{
  if ((req.method||'').toUpperCase()==='PROPFIND') return next();
  res.status(405).end();
});
app.all('/dav/:manifestId', davAuth, async (req,res,next)=>{
  if ((req.method||'').toUpperCase()==='PROPFIND') return next();
  res.status(405).end();
});
...
app.all('/dav', davAuth, async (req,res)=>{
  ...
  const mf=await query('SELECT id, original_name, size FROM manifests ORDER BY created_at DESC');
  const items=[ xmlResponse(base+'/',true) ];
  if(depth!=='0') for(const m of mf) items.push(xmlResponse(`${base}/${encodeURIComponent(m.id)}/`,true));
...
app.get('/dav/:manifestId/:filename', davAuth, async (req,res)=>{
  try{
    ...
    const name=safeFileName(m.original_name||req.params.filename);
    const ctype=m.original_mime||contentTypeFor(name);
    res.setHeader('Content-Type',ctype);
